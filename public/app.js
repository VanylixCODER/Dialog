...
function fmtDateTime(ts) { try { return new Date(ts).toLocaleString([], { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" }); } catch { return ""; } }
async function openNews(login) {
  if (!login || !profile) return;
  const { ok, data } = await api("/api/news/" + login, null, "GET");
  if (!ok || !data.owner) { notify(t("err_generic")); return; }
  $("mpModal").classList.add("hidden");
  newsRender(data);
  $("newsOverlay").classList.remove("hidden");
}
function newsRender(data) {
  clearNewsImg(); $("newsInput") && ($("newsInput").value = "");
  const ov = $("newsOverlay"); ov.dataset.login = data.owner.login; ov.dataset.owner = data.isOwner ? "1" : "0";
  $("newsAva").setAttribute("data-login", data.owner.login);
  $("newsAva").innerHTML = `<img src="${avaUrl(data.owner.login)}" onerror="this.remove()">${initials(data.owner.name)}`;
  $("newsOwnerName").textContent = data.owner.name;
  $("newsComposer").classList.toggle("hidden", !data.isOwner);
  const feed = $("newsFeed"); feed.innerHTML = "";
  (data.posts || []).forEach((p) => feed.appendChild(newsPostEl(p, data.isOwner)));
  $("newsEmpty").classList.toggle("hidden", (data.posts || []).length > 0);
}
function newsPostEl(p, pageOwner) {
  const el = document.createElement("div"); el.className = "news-post"; el.dataset.id = p.id;
  el.innerHTML =
    `<div class="np-head"><div class="avatar np-ava" data-login="${escapeHtml(p.login)}"><img src="${avaUrl(p.login)}" onerror="this.remove()">${initials(p.name)}</div>` +
    `<div class="np-meta"><span class="np-name">${escapeHtml(p.name)}</span><span class="np-time">${fmtDateTime(p.ts)}</span></div>` +
    (pageOwner ? `<button class="np-del" title="${t("delete")}">✕</button>` : "") + `</div>` +
    (p.text ? `<div class="np-text">${formatMessage(p.text)}</div>` : "") +
    (p.media ? `<div class="np-media"><img src="${p.media}" alt="" loading="lazy"></div>` : "") +
    `<div class="np-foot"><button class="np-comments-toggle">💬 <span class="npc-count">${p.comments || 0}</span> <span data-i18n="news_comments">Comments</span></button></div>` +
    `<div class="np-thread hidden"></div>`;
  return el;
}
function newsCommentEl(c) {
  const el = document.createElement("div"); el.className = "news-comment"; el.dataset.id = c.id;
  const canDel = c.login === profile.login || $("newsOverlay").dataset.owner === "1";
  el.innerHTML =
    `<div class="avatar nc-ava" data-login="${escapeHtml(c.login)}"><img src="${avaUrl(c.login)}" onerror="this.remove()">${initials(c.name)}</div>` +
    `<div class="nc-body"><div class="nc-top"><span class="nc-name">${escapeHtml(c.name)}</span><span class="nc-time">${fmtDateTime(c.ts)}</span>${canDel ? `<button class="nc-del" title="${t("delete")}">✕</button>` : ""}</div><div class="nc-text">${formatMessage(c.text)}</div></div>`;
  return el;
}
async function newsToggleThread(post, id) {
  const thread = post.querySelector(".np-thread");
  if (!thread.classList.contains("hidden")) { thread.classList.add("hidden"); return; }
  thread.classList.remove("hidden");
  if (thread.dataset.loaded === "1") return;
  thread.innerHTML = `<div class="nc-loading">${t("loading")}</div>`;
  const { ok, data } = await api("/api/news/post/" + id + "/comments", null, "GET");
  thread.innerHTML = "";
  const list = document.createElement("div"); list.className = "nc-list";
  if (ok) data.forEach((c) => list.appendChild(newsCommentEl(c)));
  thread.appendChild(list);
  const box = document.createElement("div"); box.className = "nc-input-box";
  const input = document.createElement("input"); input.className = "nc-input field"; input.maxLength = 2000; input.placeholder = t("news_comment_ph");
  const send = document.createElement("button"); send.className = "nc-send btn-primary"; send.textContent = t("t_send");
  box.appendChild(input); box.appendChild(send); thread.appendChild(box);
  const doSend = async () => {
    const txt = input.value.trim(); if (!txt) return;
    send.disabled = true;
    const r = await api("/api/news/post/" + id + "/comment", { text: txt });
    send.disabled = false;
    if (!r.ok) { notify(t("err_generic")); return; }
    input.value = ""; list.appendChild(newsCommentEl(r.data.comment));
    const cnt = post.querySelector(".npc-count"); if (cnt) cnt.textContent = (+cnt.textContent || 0) + 1;
    list.scrollTop = list.scrollHeight;
  };
  send.onclick = doSend;
  input.onkeydown = (ev) => { if (ev.key === "Enter") { ev.preventDefault(); doSend(); } };
  thread.dataset.loaded = "1";
}
function clearNewsImg() { newsImgData = null; const f = $("newsFile"); if (f) f.value = ""; const p = $("newsImgPreview"); if (p) p.classList.add("hidden"); }
// Wiring
$("newsBack") && ($("newsBack").onclick = () => $("newsOverlay").classList.add("hidden"));
$("newsOverlay") && $("newsOverlay").addEventListener("click", (e) => { if (e.target === $("newsOverlay")) $("newsOverlay").classList.add("hidden"); });
document.addEventListener("keydown", (e) => { if (e.key === "Escape" && $("newsOverlay") && !$("newsOverlay").classList.contains("hidden")) $("newsOverlay").classList.add("hidden"); });
$("newsAttach") && ($("newsAttach").onclick = () => $("newsFile").click());
$("newsFile") && ($("newsFile").addEventListener("change", () => {
  const f = $("newsFile").files[0]; if (!f) return;
  if (f.size > 5 * 1024 * 1024) { notify(t("news_img_big")); $("newsFile").value = ""; return; }
  const r = new FileReader(); r.onload = () => { newsImgData = r.result; $("newsImgEl").src = r.result; $("newsImgPreview").classList.remove("hidden"); }; r.readAsDataURL(f);
}));
$("newsImgClear") && ($("newsImgClear").onclick = clearNewsImg);
$("newsPost") && ($("newsPost").onclick = async () => {
  const text = $("newsInput").value.trim();
  if (!text && !newsImgData) return;
  $("newsPost").disabled = true;
  const { ok, data } = await api("/api/news", { text, media: newsImgData });
  $("newsPost").disabled = false;
  if (!ok) { notify(t("err_generic")); return; }
  $("newsInput").value = ""; clearNewsImg(); $("newsEmpty").classList.add("hidden");
  $("newsFeed").insertBefore(newsPostEl(data.post, true), $("newsFeed").firstChild);
});
$("newsFeed") && $("newsFeed").addEventListener("click", async (e) => {
  const post = e.target.closest(".news-post"); if (!post) return;
  const id = post.dataset.id;
  if (e.target.closest(".np-del")) {
    if (!confirm(t("news_del_confirm"))) return;
    const { ok } = await api("/api/news/post/" + id, null, "DELETE");
    if (ok) { post.remove(); if (!$("newsFeed").children.length) $("newsEmpty").classList.remove("hidden"); }
    return;
  }
  if (e.target.closest(".nc-del")) {
    const cel = e.target.closest(".news-comment"); const cid = cel && cel.dataset.id; if (!cid) return;
    const { ok } = await api("/api/news/comment/" + cid, null, "DELETE");
    if (ok) { cel.remove(); const cnt = post.querySelector(".npc-count"); if (cnt) cnt.textContent = Math.max(0, (+cnt.textContent || 1) - 1); }
    return;
  }
  const ava = e.target.closest("[data-login]");
  if (ava && ava.dataset.login && ava.dataset.login !== profile.login) { openMiniProfile(ava.dataset.login); return; }
  if (e.target.closest(".np-comments-toggle")) newsToggleThread(post, id);
});
$("myNewsBtn") && ($("myNewsBtn").onclick = () => openNews(profile.login));
$("mpPage") && ($("mpPage").onclick = () => { const login = $("mpAva").getAttribute("data-login"); if (login) openNews(login); });
socket.on("news-comment", ({ fromName }) => { if (!isDnd()) notify(t("news_comment_ping", { name: fromName || "" })); });
